...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full px-4 md:px-10 lg:px-16 py-3 transition-all duration-300 relative">
      <header className="flex items-center justify-between whitespace-nowrap px-6 py-4 rounded-2xl bg-surface-container-highest/40 backdrop-blur-2xl border border-outline-variant/15 shadow-[0_8px_32px_rgba(0,0,0,0.08),inset_0_2px_1px_rgba(255,255,255,0.5),inset_0_-1px_1px_rgba(0,0,0,0.05)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.4),inset_0_1px_1px_rgba(255,255,255,0.1),inset_0_-1px_1px_rgba(0,0,0,0.2)]">
        ...
        <div className="hidden md:flex flex-1 justify-end gap-8 items-center">
          <div className="flex items-center gap-8">
            <a
              className="text-on-surface-variant hover:text-primary transition-colors text-md font-semibold"
              href="#"
            >
              About
            </a>
          ...
          <ThemeToggle />
          <button className="flex items-center justify-center rounded-xl h-10 px-5 bg-gradient-to-r from-[#4a50c5] to-[#00b08a] text-white text-md font-bold hover:shadow-lg transition-all active:scale-95">
            <span>Sign In</span>
          </button>
        </div>
        ...
        <div className="flex items-center gap-3 md:hidden">
          ...
          <button 
            ...
            className="p-2 text-on-surface-variant hover:text-on-surface hover:bg-outline-variant/10 rounded-xl transition-all"
      ...
      <div 
        className={`absolute top-full left-4 right-4 mt-2 p-6 rounded-3xl bg-white/95 dark:bg-[#121215]/95 backdrop-blur-3xl border border-black/5 dark:border-white/10 shadow-2xl transition-all duration-300 origin-top transform md:hidden flex flex-col gap-8 ${
          isMobileMenuOpen 
      ...
      >
        <div className="flex flex-col gap-6">
          <a href="#" onClick={() => setIsMobileMenuOpen(false)} className="text-gray-800 dark:text-gray-300 hover:text-primary transition-colors text-xl font-headline font-semibold">
            About
          </a>
          <a href="#" onClick={() => setIsMobileMenuOpen(false)} className="text-gray-800 dark:text-gray-300 hover:text-primary transition-colors text-xl font-headline font-semibold">
            Features
        ...
        <div className="flex flex-col gap-4 mt-2 mb-2">
          <button onClick={() => setIsMobileMenuOpen(false)} className="flex w-full items-center justify-center rounded-2xl h-14 bg-gradient-to-r from-[#4a50c5] to-[#00b08a] text-white text-lg font-bold hover:shadow-lg transition-all active:scale-95 shadow-[0_4px_20px_rgba(74,80,197,0.3)]">
            Sign In To RepoHawk
          </button>
        </div>
      </div>
